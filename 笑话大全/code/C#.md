...
**使用该产品前，您需要通过 [https://www.apishop.net/#/api/detail/?productID=122](https://www.apishop.net/#/api/detail/?productID=122 "https://www.apishop.net/#/api/detail/?productID=122") 申请API服务**
...
namespace apishop_sdk
{
	class Program
	{
		...
		{
			string method = "POST";
			string url = "https://api.apishop.net/common/joke/getJokes";
			Dictionary<string, string> param = new Dictionary<string, string>();			
			param.Add("page", ""); //页码，默认第一页			
			param.Add("pageSize", ""); //每页条数（最大20）			
			param.Add("updateTime", ""); //更新时间（不填从最早的数据开始），时间格式“2017-08-31 00:00:00”			
			param.Add("delay", ""); //数据的更新日期相对于updateTime（1：before[之前]，2：after[之后]）		
