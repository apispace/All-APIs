...
namespace apishop_sdk
{
	class Program
	{
		...
		{
			string method = "POST";
			string url = "https://api.apishop.net/common/checkcode/recognizeSimpleCode";
			Dictionary<string, string> param = new Dictionary<string, string>();			
			param.Add("codeType", ""); //验证码类型 1001: 数字+字母类型， 1002: 纯英文类型，1003: 纯数字类型，1004:计算题类型（使用multipart/form-data方式请求）			
			param.Add("length", ""); //验证码长度（使用multipart/form-data方式请求）			
			param.Add("image", ""); //验证码图片，图片大小需要小于100KB（使用multipart/form-data方式请求）		
