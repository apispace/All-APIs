...
namespace apishop_sdk
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			param.Add("content", ""); //内容			
			param.Add("size", ""); //图片大小（范围：1~10)，默认为4【注：手机扫描，size为4即可】			
			param.Add("level", ""); //容错（0:L,1:M,:2:H,3:Q），默认为0			
			param.Add("margin", ""); //边框大小（范围：1~10），默认为3		
...
namespace apishop_sdk
{
	class Program
	{
		...
		{
			string method = "POST";
			string url = "https://api.apishop.net/common/qrcode/qrCode2Content";
			Dictionary<string, string> param = new Dictionary<string, string>();			
			param.Add("image", ""); //二维码图片文件		
